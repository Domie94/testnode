...
const getEvents = async () => {
    try {
        ...
        const sqlQueries = await utils.loadSqlQueries('company');
        const companylistss = await pool.request().query(sqlQueries.companylist);
        return companylistss.recordset;
    } catch (error) {
...
const getById = async(CompanyId) => {
    try {
        ...
        const event = await pool.request()
                            .input('CompanyId', sql.NVarChar(255), CompanyId)
